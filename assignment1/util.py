...
def word2index(tokens, dataset_raw):
    dataset = []
    max_length = 0
    for i in range(len(dataset_raw)):
        sentence = dataset_raw[i][0]
        max_length = max(max_length, len(sentence))
        dataset.append(([tokens[word] for word in sentence], dataset_raw[i][1]))
        
    return max_length, dataset, dataset_raw
