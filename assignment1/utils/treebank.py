...
class StanfordSentiment:
    ...
    def getRandomContext(self, C=5):
        ...
        sentID = random.randint(0, len(allsent) - 1) # randomly choose a sentence
        sent = allsent[sentID]
        wordID = random.randint(0, len(sent) - 1) # randomly choose a word
    ...
    def sent_labels(self):
        ...
        with open(self.path + "/dictionary.txt", "r") as f:
            for line in f:
                ...
                splitted = line.split("|")
                dictionary[splitted[0].encode('utf-8').decode('latin1').lower().replace('\xa0', '')] = int(splitted[1])
                dictionary[splitted[0].encode('utf-8').decode('latin1').lower().replace('\xa0', ' ')] = int(splitted[1])
                phrases += 1
